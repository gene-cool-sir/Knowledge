...
-   `npm install 包名 --save`  **安装包并将其添加到依赖中**，一般不传node\_modules，太大、不能保证是最新的，有了依赖，传package.json直接`npm install`可以下载当前项目所依赖的包
-   `npm uninstall 包名`  卸载包
-   `npm install 文件路径`  从本地安装
-   `npm install 包名 -registry=地址`  从镜像源安装包
-   `npm config set registry 地址`  设置镜像源

淘宝镜像：[https://npm.taobao.org](https://npm.taobao.org "https://npm.taobao.org")
...
-   Node 在使用模块名字来引入模块时，它会首先在当前目录的 node\_modules 中寻找是否含有该模块，如果有则直接使用，如果没有则去上一级目录的 node\_modules 中寻找，如果有则直接使用，如果没有则再去上一级目录寻找，知道找到为止，如果找到磁盘根目录仍没有，则报错。
