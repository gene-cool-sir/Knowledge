...
用了好久的黑苹果，但是那电脑已经有点 hold 不住了，所以干脆换了 MacBook Pro，由于 M2 相比于 M1 提升并没有很大，而且最新的实在太贵了，所以直接在苹果官网找的[官翻机](https://www.apple.com.cn/shop/refurbished/mac/macbook-pro "官翻机")。

最后选中的是 14 英寸 MacBook Pro Apple M1 Pro 芯片（配备 10 核中央处理器和 16核图形处理器）- 银色，32 GB 内存 + 1 T 存储空间

![](https://cdn.wallleap.cn/img/pic/illustration/202305291740275.png)
...
![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005904.png)
...
> 也可以直接看这个 [https://zhuanlan.zhihu.com/p/474801204](https://zhuanlan.zhihu.com/p/474801204 "https://zhuanlan.zhihu.com/p/474801204")
...
好用的软件包管理器，官网为：[Homebrew](https://brew.sh/ "Homebrew")（可以直接到官网首页搜索框搜软件，之后直接复制到终端运行）
...
![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005905.png)
...
![](https://cdn.wallleap.cn/img/pic/illustration/202305291801410.png)

在例外中默认是黑名单模式，加上不需要平滑或反转的应用

![](https://cdn.wallleap.cn/img/pic/illustration/202305291803342.png)

### 程序坞

-   默认的 Dock 栏效果：
    -   呆呆傻傻，没有动画，显示最近使用的应用程序（关掉了还会占位置）
    -   可以先记住恢复默认 Dock 的命令：`defaults delete com.apple.dock; killall Dock`![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005907.png)
-   系统设置-程序坞与菜单栏，设置图标大小和动画（长按拖动分割线可以快速调整图标大小、按住 shift 键长按分割线上下左右拖动快速调整位于屏幕上的位置、右键分割线可以快速设置隐藏等）

    ![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005908.png)

    ![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005909.png)
-   添加和删除图标：
...
-   添加空白占位图标，右键选择从程序坞中移除可以删掉
    ...
    killall Dock
    ```
    ![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005910.png)
...
-   系统设置里可以设置是否在菜单栏显示，一般在底部
    ...
    ![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005911.png)
...
-   菜单栏-访达-偏好设置（大部分 app 的首选项设置快捷键都是 \<kbd>Command\</kbd> + \<kbd>,\</kbd>）
    ...
    -   设置显示隐藏文件：在终端输入 `defaults write com.apple.Finder AppleShowAllFiles true; killall Finder`，或者在访达中使用快捷键 \<kbd>⌘\</kbd> + \<kbd>Shift\</kbd> + \<kbd>.\</kbd> 控制隐藏文件的显示或隐藏
        ![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005912.jpeg)
-   工具栏设置，右击工具栏选择自定工具栏，根据自己喜好添加删除排序

    ![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005913.png)
-   显示，菜单栏-显示
-   常用操作：
...
推荐一个应用分享站：[精品MAC应用分享 (xclient.info)](https://xclient.info/ "精品MAC应用分享 (xclient.info)")

付费软件，我一般到这上面购买：[麦软网](https://www.mairuan.com/ "麦软网")、[数码荔枝)](https://lizhi.shop/ "数码荔枝)")

M1 可以直接安装移动端 APP，对于 App Store 中没有的移动端 APP，可以到网上找下砸壳了的 IPA 文件，然后到这个网站中安装：[https://appdb.to/my/devices](https://appdb.to/my/devices "https://appdb.to/my/devices")
...
之前有写过 Rime 的文章，可以去看下：[个人输入法调教之 Rime](https://myblog.wallleap.cn/#/post/93 "个人输入法调教之 Rime")

个人是把系统自带的中文输入法删除了，只留下了 ABC 和鼠须管

![](https://cdn.wallleap.cn/img/pic/illustration/202305291816704.png)
...
可以去 [Raycast - Store](https://www.raycast.com/store "Raycast - Store") 找好用的扩展插件
...
win 下之前使用2345看图王绿色版非常符合，后面找到[蓝山看图王](https://www.lanshan.com/pic/ "蓝山看图王")也不错
...
<https://zh.altapps.net/soft/keycastr>
...
ClashX Pro 直接到这里下载：[App Center](https://install.appcenter.ms/users/clashx/apps/clashx-pro/distribution_groups/public "App Center")
...
一般机场都会提供 ClashX 的订阅，如果没有可以转换后一键导入到 Clash：[Subscription Converter](https://sub-web.opo.repl.co/ "Subscription Converter")
...
App 可以在 [homebrew-cask — Homebrew Formulae](https://formulae.brew.sh/cask/ "homebrew-cask — Homebrew Formulae") 里找有没有，Cli 工具可以在 [homebrew-core — Homebrew Formulae](https://formulae.brew.sh/formula/ "homebrew-core — Homebrew Formulae") 找有没有
...
-   [Flomo x Pake](https://github.com/tw93/Pake "Flomo x Pake")
-   [Flux](https://justgetflux.com/ "Flux")
-   Reeder（国区没有）
-   PDF Expert
-   [uPic](https://github.com/gee1k/uPic "uPic")
-   [ChatGPT x Tauri](https://github.com/lencx/ChatGPT "ChatGPT x Tauri")
-   [Microsoft 365](https://apps.apple.com/cn/app-bundle/microsoft-365/id1450038993?mt=12 "Microsoft 365")
-   [以方·iFonts字体助手](https://ifonts.com/client "以方·iFonts字体助手")
-   [Neat Download Manager](https://www.neatdownloadmanager.com/index.php/en/ "Neat Download Manager")
...
参考 [Karabiner-Element 配置 F19 键 - HackMD](https://hackmd.io/@UXqYDTxCTie91Shvsppqyw/rk4u9i-pG?type=view "Karabiner-Element 配置 F19 键 - HackMD") 。在 [Karabiner-Elements complex\_modifications rules](https://pqrs.org/osx/karabiner/complex_modifications/ "Karabiner-Elements complex_modifications rules") 搜「Change caps\_lock key」，import 后只保留一条和 F19 相关的，然后在命令行里编辑「\~/.config/karabiner/karabiner.json」，把刚才那条规则的「caps\_lock」换成「right\_command」（两处）。这样你就把基本不会用到的「右⌘」废物利用变成了「F19」键，然后你的快捷键组合会多很多。
...
先配置 iTerm2，这是[效果图](https://img.alicdn.com/imgextra/i1/O1CN01PPttEm1mCx3bddVjX_!!6000000004919-2-tps-2374-1532.png "效果图")。1）Appearance 里，General 的 Theme 选「Minimal」，Pane 里不要「Show per-pane title bar with split panes」，Dimming 里选上第一和第三个，2）Profiles 里，Working Directory 里选「Reuse previous session's directory」。

安装 zsh 和 [starship](https://starship.rs/ "starship")，starship 是 rust 写的 prompt 工具，极快。
...
配置 \~/.zshrc，我的配置如下（略做删减）。这里有些 alias 是以 `,` 开头的，因为这样你敲 `,` 然后按「Tab」就可以[看到所有自己定义的命令](https://img.alicdn.com/imgextra/i3/O1CN01XUDvg01ZHRwduLZSo_!!6000000003169-0-tps-1422-194.jpg "看到所有自己定义的命令")了。为啥有些没有加 `,` ？历史原因… 因为其他都用习惯了就不改了。
...
我的配置见[图](https://img.alicdn.com/imgextra/i3/O1CN01PWmDZN24797TUfPbL_!!6000000007343-0-tps-594-1918.jpg "图")，快捷键供参考。

8、安装字体

编程字体我用 [Monolisa](https://www.monolisa.dev/ "Monolisa")，之前还用过 [Source Code Pro](https://github.com/adobe-fonts/source-code-pro "Source Code Pro")、[Dank Mono](https://dank.sh/ "Dank Mono") 和 [Operator Mono](https://www.typography.com/fonts/operator/overview "Operator Mono")。此外我还安装了[霞鹜文楷](https://github.com/lxgw/LxgwWenKai "霞鹜文楷")和阿里普惠体，霞鹜文楷我用在了语雀等文档站和 Obsidian 里。
...
主题是 [Nako - Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=kettanaito.nako "Nako - Visual Studio Marketplace")。
...
要稍微做下配置。1）添加 Block App，我加了钉钉、Reeder，2）Block Website 里我把内置的全删了，留了[这些](https://img.alicdn.com/imgextra/i3/O1CN017Pt2Y827DxrmBSPh4_!!6000000007764-2-tps-1448-1408.png "这些")，3）Block App 的配置要选「强制退出」，「隐藏」的模式在 Mac 13.2 下有问题，不会生效或者表现奇怪。

15、Bob。

我的快捷键是「F19+A」划词翻译，「F19+S」截图翻译。插件装了 [bob-plugin-deeplx](https://github.com/clubxdev/bob-plugin-deeplx "bob-plugin-deeplx") 和 [bobplugin-google-translate](https://github.com/roojay520/bobplugin-google-translate "bobplugin-google-translate")。文本翻译我加了 Deepl X、有道、阿里、金山词霸，文本识别我用腾讯 OCR。
...
登录 Google 账号后所有东西就都同步过来了，除了 Tampermonkey 的自定义脚本。但简单 Google 后也找到了办法，我参考 [extract\_tampermonkey\_script.py · GitHub](https://gist.github.com/derjanb/9f6c10168e63c3dc3cf0 "extract_tampermonkey_script.py · GitHub") 把 `/Users/%YOUR_USERNAME%/Library/Application\ Support/Google/Chrome/Default/Local\ Extension\ Settings/dhdgffkkebhmkfjojejmpbldmpobfkfo` 这个文件夹下的内容复制到新电脑后就能用了。
...
把所有 Markdown 文件[改成用 iA Writer 打开](https://img.alicdn.com/imgextra/i1/O1CN01YZ2Osn1qtmYrCS9tw_!!6000000005554-2-tps-502-752.png "改成用 iA Writer 打开")，因为 iA Writer 又轻又好看。然后我在「系统设置 > Keyboard > Keyboard Shortcuts > App Shortcuts」中增加了一些针对 IA Writer 的快捷键配置。
...
11、执行 `defaults write -g NSWindowShouldDragOnGesture -bool true`，然后就可以按住「⌘+⌃」然后鼠标点击任意地方拖动窗口了。来源 [Moving a macOS window by clicking anywhere on it (like on Linux) · mmazzarolo.com](https://mmazzarolo.com/blog/2022-04-16-drag-window-by-clicking-anywhere-on-macos/ "Moving a macOS window by clicking anywhere on it (like on Linux) · mmazzarolo.com")，但是在 MacOS 13 下似乎失效了。

## 参考

-   [Mac Setup for Web Development 2023](https://www.robinwieruch.de/mac-setup-web-development/ "Mac Setup for Web Development 2023")
-   [My 2021 New Mac Setup](https://www.swyx.io/new-mac-setup-2021 "My 2021 New Mac Setup")
...
![](https://cdn.wallleap.cn/img/pic/illustrtion/20221009005914.jpeg)
